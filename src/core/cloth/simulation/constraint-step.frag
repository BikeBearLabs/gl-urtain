...
// to look for the neighbor we're going to check
...
void main() {
	...
	if (!pinned) {
		...
		if (otherCoord == ivec2(0, 0) ||
			otherCoord == ivec2(dimensions.x - 1, 0)) {
			otherPin = true;
		}

		if (all(greaterThanEqual(otherCoord, ivec2(0, 0))) &&
			all(lessThan(otherCoord, dimensions))) {
			...
			if (dist > uRestDistance) {
				position += diffVec * (1.0 - uRestDistance / dist) *
							(otherPin ? 1.0 : 0.5);
			}
		}
	}
